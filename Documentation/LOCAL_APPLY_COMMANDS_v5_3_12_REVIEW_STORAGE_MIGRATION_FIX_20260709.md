# NOVALYTE v5.3.12 REVIEW STORAGE MIGRATION FIX
...
$zipName = "NOVALYTE_v5.3.12_REVIEW_STORAGE_MIGRATION_FIX_20260709.zip"
$downloadZip = Join-Path $downloads $zipName
$baseZip = Join-Path $base $zipName

Write-Host "=== NOVALYTE v5.3.12 REVIEW STORAGE MIGRATION FIX ===" -ForegroundColor Cyan
...
Select-String -Path .\index.html -Pattern "v=5.3.12|Resources/Brand/novalyte.png"
Select-String -Path .\admin.html -Pattern "v=5.3.12|Resources/Brand/novalyte.png|adminConfirmModal"
Select-String -Path .\assets\js\app.js -Pattern "normalizeAndPersistPublicReviews|They responded quickly|Pnovalyte001|client-floating-nav"
Select-String -Path .\Documentation\NOVALYTE_FULL_PROJECT_HANDOVER_SUMMARY_v5_3_CHECKPOINT_20260708.txt -Pattern "V5.3.12 REVIEW STORAGE MIGRATION FIX"
...
if ($changes.Trim().Length -gt 0) {
    git add -A
    git add -f .\Resources\Brand\novalyte.png
    git commit -m "Novalyte v5.3.12 review storage migration fix"
...
Start-Process "https://potechto.github.io/boosting-menu/index.html?v=53121"
Start-Process "https://potechto.github.io/boosting-menu/admin.html?v=53121"

Write-Host "Live marker check. GitHub Pages may need a few seconds to update." -ForegroundColor Cyan
Start-Sleep -Seconds 10
try {
    $liveIndex = Invoke-WebRequest "https://potechto.github.io/boosting-menu/index.html?v=53121" -UseBasicParsing
    $liveAdmin = Invoke-WebRequest "https://potechto.github.io/boosting-menu/admin.html?v=53121" -UseBasicParsing

    if ($liveIndex.Content -notmatch "v=5.3.12" -or $liveIndex.Content -notmatch "normalizeAndPersistPublicReviews") {
        Write-Host "WARNING: live index may still be cached/not updated. Wait 30 seconds then reopen ?v=53121." -ForegroundColor Yellow
    } else {
        Write-Host "Live index marker v5.3.12 + review migration detected." -ForegroundColor Green
    }

    if ($liveAdmin.Content -notmatch "v=5.3.12") {
        Write-Host "WARNING: live admin may still be cached/not updated. Wait 30 seconds then reopen ?v=53121." -ForegroundColor Yellow
    } else {
        Write-Host "Live admin marker v5.3.12 detected." -ForegroundColor Green
    }
} catch {
    Write-Host "Live marker check failed or GitHub Pages is still updating. Reopen the ?v=53121 links after 30 seconds." -ForegroundColor Yellow
}
...
Write-Host "DONE. v5.3.12 applied, checked, local pages opened, GitHub pushed if changes existed, live pages opened, ZIP cleaned." -ForegroundColor Green
```
